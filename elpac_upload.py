"""ELPAC test settings upload — Aeries export to TOMS."""
...
ROLE_TEXT = "Site ELPAC Coordinator"
TEMPLATE_FILENAME = "ELPAC_Upload_Stu_Accom_Template.xlsx"
...
def main():
    ...
    try:
        ...
        toms_navigate_to_upload(driver, upload_type_value='/mt/dt/uploadElpacAccoms.htm')
        ...
        aeries_export = os.path.join(downloads_dir, f"ELPACTestSettings{tomorrow_str}.csv")

        if not os.path.exists(aeries_export):
            print(f"Aeries export not found: {aeries_export}")
            print("Run aeries_elpac_export.py first.")
            return
